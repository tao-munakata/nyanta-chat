...
にゃんこ先生の問診チャットを起点にした、テーマ切り替え型の猫チャットです。訪問診療・在宅医療の問診票に加えて、雑談、気持ち整理、匿名相談の横展開を同じVPS上で動作確認できます。
...
- Production path: configurable with `BASE_PATH` (`/nyanko` by default, `/` for root-domain deployment)
- Service paths: `/medical`, `/smalltalk`, `/mood`, `/secret` when `BASE_PATH=/`; `/nyanko/medical`, `/nyanko/smalltalk`, `/nyanko/mood`, `/nyanko/secret` when `BASE_PATH=/nyanko`

## 主な機能

- ルートまたは `BASE_PATH` でサービス一覧を表示
- `nyanko-medical`: 問診質問25問、7カテゴリ
- `nyanko-smalltalk`: 猫の顔を見ながら雑談できる自由チャット
- `nyanko-mood`: 落ち込みや不安をやさしく整理する自由チャット
- `nyanko-secret`: 人には言いにくい話題を匿名前提で整理する自由チャット
...
http://localhost:3000/nyanko/medical
http://localhost:3000/nyanko/smalltalk
http://localhost:3000/nyanko/mood
http://localhost:3000/nyanko/secret
...
http://localhost:3001/nyanko/medical
http://localhost:3001/nyanko/smalltalk
http://localhost:3001/nyanko/mood
http://localhost:3001/nyanko/secret
